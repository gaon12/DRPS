...
const CprStep1 = ({ onNext, delay = 3000 }) => { // delay 기본값: 3000ms (3초)
      ...
      (async () => {
        try {
          ...
          const { sound } = await Audio.Sound.createAsync(
            require('../assets/cprstep1.mp3'), // MP3 파일 경로
            { shouldPlay: true }
          );
  ...
  return (
    <View style={styles.container}>
      <Image
    source={require('./cprstepimages/cpr1.png')} // 이미지 파일 경로
    style={{ width: 300, height: 300, marginBottom: 10 }} // 이미지 스타일
  />
      <View style={styles.textContainer}>
      
        <Text style={styles.text}>가슴압박 30회</Text>
      </View>
      {countdown > 0 ? (

...
export default function ApiScreen({ route, navigation }) {
	...
	const handlePinchGesture = (event) => {
		if (event.nativeEvent.scale) {
			...
			setScale(Math.max(1, Math.min(newScale, 3))); // 최소 1, 최대 3배까지 제한
		}
	};

	// 제스처가 종료되었을 때 lastScale 갱신
	const handlePinchGestureStateChange = (event) => {
		if (event.nativeEvent.oldState === 4) { // 4는 END 상태를 나타냅니다.
				...
				{pages.map((page, index) => (
					...
					>
						...
						>
							<Image
								source={page}
								placeholder={{ thumbhash: thumbhashList[index] }} // 각 페이지에 해당하는 thumbhash를 플레이스홀더로 전달
